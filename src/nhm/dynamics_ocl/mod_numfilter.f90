...
!  real(8) :: cfact = 2.0D0    Y.Niwa 080130
  ...
  logical, private, save :: debug_output = .true.
...
!                Kh_coef_lap1_h_pl(n,k,l) = gamma_h_lap1
  ...
  subroutine numfilter_divdamp( &
       ...
       gdx,    gdx_pl,    & !--- [OUT] : (grad div)_x ( gam2 X G^{1/2} )
       gdy,    gdy_pl,    & !--- [OUT] : (grad div)_y ( gam2 X G^{1/2} )
       gdz,    gdz_pl,    & !--- [OUT] : (grad div)_z ( gam2 X G^{1/2} )
       gdvz,   gdvz_pl    ) !--- [OUT] : (grad div)_w ( gam2 X G^{1/2} )
    use mod_adm, only: &
       ADM_prc_me,  &
       ADM_prc_pl,  &
       ADM_gall,    &
    ...
    real(8), intent(out) :: gdvz     (ADM_gall,   ADM_kall,ADM_lall   )
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             vtmp_pl(g,k,l,d) = -vtmp2_pl(g,k,l,d)
          enddo
          enddo
          enddo
          enddo
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdx_pl(g,k,l) = divdamp_coef_pl(g,k,l) * vtmp2_pl(g,k,l,1)
       enddo
       enddo
       enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdy_pl(g,k,l) = divdamp_coef_pl(g,k,l) * vtmp2_pl(g,k,l,2)
       enddo
       enddo
       enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdz_pl(g,k,l) = divdamp_coef_pl(g,k,l) * vtmp2_pl(g,k,l,3)
       enddo
       enddo
       enddo
    endif
...
call timer_end(2550)
  ...
  subroutine numfilter_divdamp_2d( &
       ...
       gdx,    gdx_pl,    & !--- [OUT] : (grad div)_x ( gam2 X G^{1/2} )
       gdy,    gdy_pl,    & !--- [OUT] : (grad div)_y ( gam2 X G^{1/2} )
       gdz,    gdz_pl     ) !--- [OUT] : (grad div)_w ( gam2 X G^{1/2} )
    use mod_adm, only: &
       ADM_prc_me,  &
       ADM_prc_pl,  &
       ADM_gall,    &
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdx_pl(g,k,l) = divdamp_2d_coef_pl(g,k,l) * vtmp2_pl(g,k,l,1)
       enddo
       enddo
       enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdy_pl(g,k,l) = divdamp_2d_coef_pl(g,k,l) * vtmp2_pl(g,k,l,2)
       enddo
       enddo
       enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          gdz_pl(g,k,l) = divdamp_2d_coef_pl(g,k,l) * vtmp2_pl(g,k,l,3)
       enddo
       enddo
       enddo
    endif
...
!OCL XFILL
       do g = 1, ADM_gall
          rhog(g,k,l) = rho(g,k,l) * VMTR_GSGAM2(g,k,l)
       enddo
       enddo
    enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          rhog_pl(g,k,l) = rho_pl(g,k,l) * VMTR_GSGAM2_pl(g,k,l)
       enddo
       enddo
    enddo
!OCL SERIAL
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          vtmp_pl(g,k,l,6) = rho_pl (g,k,l) - rho_bs_pl(g,k,l)
       enddo
       enddo
    enddo
...
!OCL XFILL
          do g = 1, ADM_gall
             vtmp_lap1(g,k,l,nq) = vtmp(g,k,l,nq)
          enddo
          enddo
          enddo
       enddo
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             vtmp_lap1_pl(g,k,l,nq) = vtmp_pl(g,k,l,nq)
          enddo
          enddo
          enddo
       enddo
...
!OCL XFILL
          do g = 1, ADM_gall
             wk(g,k,l) = rhog(g,k,l) * CNST_CV * KH_coef(g,k,l)
          enddo
          enddo
          enddo
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             wk_pl(g,k,l) = rhog_pl(g,k,l) * CNST_CV * KH_coef_pl(g,k,l)
          enddo
          enddo
          enddo
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             wk_pl(g,k,l) = rhog_pl(g,k,l) * hdiff_fact_rho * KH_coef_pl(g,k,l)
          enddo
          enddo
          enddo
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             vtmp_pl(g,k,l,nq) = -vtmp2_pl(g,k,l,nq)
          enddo
          enddo
          enddo
       enddo
...
!OCL XFILL
       do g = 1, ADM_gall
          wk(g,k,l) = rhog(g,k,l) * CNST_CV * KH_coef_lap1(g,k,l)
       enddo
       enddo
       enddo
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          wk_pl(g,k,l) = rhog_pl(g,k,l) * CNST_CV * KH_coef_lap1_pl(g,k,l)
       enddo
       enddo
       enddo
...
!OCL XFILL
       do g = 1, ADM_gall_pl
          wk_pl(g,k,l) = rhog_pl(g,k,l) * hdiff_fact_rho * KH_coef_lap1_pl(g,k,l)
       enddo
       enddo
       enddo
...
!OCL XFILL
          do g = 1, ADM_gall
             vtmp_lap1(g,k,l,nq) = -vtmp2(g,k,l,nq)
          enddo
          enddo
          enddo
       enddo
...
!OCL XFILL
          do g = 1, ADM_gall_pl
             vtmp_lap1_pl(g,k,l,nq) = -vtmp2_pl(g,k,l,nq)
          enddo
          enddo
          enddo
       enddo
