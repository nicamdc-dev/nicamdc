...
!                Kh_coef_lap1_h_pl(n,k,l) = gamma_h_lap1
  ...
  subroutine numfilter_numerical_hdiff( &
    ...
    real(8), intent(inout) :: tend    (ADM_gall,   ADM_kall,ADM_lall   ,nmax_tendency)
    real(8), intent(inout) :: tend_pl (ADM_gall_pl,ADM_kall,ADM_lall_pl,nmax_tendency)
    real(8), intent(inout) :: tendq   (ADM_gall,   ADM_kall,ADM_lall   ,TRC_VMAX)
    real(8), intent(inout) :: tendq_pl(ADM_gall_pl,ADM_kall,ADM_lall_pl,TRC_VMAX)
    ...
    vtmp   (:,:,:,I_RHOG  ) = rho    (:,:,:) - rho_bs   (:,:,:)
    vtmp   (:,:,:,I_RHOGVX) = vx     (:,:,:)
    vtmp   (:,:,:,I_RHOGVY) = vy     (:,:,:)
    vtmp   (:,:,:,I_RHOGVZ) = vz     (:,:,:)
    vtmp   (:,:,:,I_RHOGW ) = w      (:,:,:)
    vtmp   (:,:,:,I_RHOGE ) = tem    (:,:,:) - tem_bs   (:,:,:)

    vtmp_pl(:,:,:,I_RHOG  ) = rho_pl (:,:,:) - rho_bs_pl(:,:,:)
    vtmp_pl(:,:,:,I_RHOGVX) = vx_pl  (:,:,:)
    vtmp_pl(:,:,:,I_RHOGVY) = vy_pl  (:,:,:)
    vtmp_pl(:,:,:,I_RHOGVZ) = vz_pl  (:,:,:)
    vtmp_pl(:,:,:,I_RHOGW ) = w_pl   (:,:,:)
    vtmp_pl(:,:,:,I_RHOGE ) = tem_pl (:,:,:) - tem_bs_pl(:,:,:)
    ...
    do p = 1, lap_order_hdiff
       ! for momentum
       call OPRT_laplacian( vtmp2(:,:,:,I_RHOGVX), vtmp2_pl(:,:,:,I_RHOGVX), & !--- [OUT]
                            vtmp (:,:,:,I_RHOGVX), vtmp_pl (:,:,:,I_RHOGVX)  ) !--- [IN]
 
       call OPRT_laplacian( vtmp2(:,:,:,I_RHOGVY), vtmp2_pl(:,:,:,I_RHOGVY), & !--- [OUT]
                            vtmp (:,:,:,I_RHOGVY), vtmp_pl (:,:,:,I_RHOGVY)  ) !--- [IN]
 
       call OPRT_laplacian( vtmp2(:,:,:,I_RHOGVZ), vtmp2_pl(:,:,:,I_RHOGVZ), & !--- [OUT]
                            vtmp (:,:,:,I_RHOGVZ), vtmp_pl (:,:,:,I_RHOGVZ)  ) !--- [IN]
 
       call OPRT_laplacian( vtmp2(:,:,:,I_RHOGW), vtmp2_pl(:,:,:,I_RHOGW), & !--- [OUT]
                            vtmp (:,:,:,I_RHOGW), vtmp_pl (:,:,:,I_RHOGW)  ) !--- [IN]
       ! for scalar
       if ( p == lap_order_hdiff ) then
          ...
          if ( trim(hdiff_type) == "NONLINEAR1" ) then
             do l = 1, ADM_lall
                do k = 1, ADM_kall
                do g = 1, ADM_gall
                   d2T_dx2 = abs(vtmp(g,k,l,I_RHOGE)) / T0 * horiz_dx2
                ...
                do g = 1, ADM_gall
                   KH_coef_h(g,ADM_kmin-1,l) = 0.D0
                   KH_coef_h(g,ADM_kmin,  l) = 0.D0
                   KH_coef_h(g,ADM_kmax,  l) = 0.D0
                enddo
             enddo
             ...
             do l = 1, ADM_lall_pl
                do k = 1, ADM_kall
                do g = 1, ADM_gall_pl
                   d2T_dx2 = abs(vtmp_pl(g,k,l,I_RHOGE)) / T0 * horiz_dx2
                   coef    = cfact * ( horiz_dx2 * horiz_dx2 ) / TIME_DTL * d2T_dx2
                ...
                do g = 1, ADM_gall_pl
                   KH_coef_h_pl(g,ADM_kmin-1,l) = 0.D0
                   KH_coef_h_pl(g,ADM_kmin,  l) = 0.D0
                   KH_coef_h_pl(g,ADM_kmax,  l) = 0.D0
                enddo
             enddo
          endif ! nonlinear1
          ...
          call OPRT_diffusion( vtmp2(:,:,:,I_RHOGE), vtmp2_pl(:,:,:,I_RHOGE), &
                               vtmp (:,:,:,I_RHOGE), vtmp_pl (:,:,:,I_RHOGE), &
                               wk   (:,:,:),         wk_pl   (:,:,:)         )

          wk   (:,:,:) = rhog   (:,:,:) * hdiff_fact_rho * KH_coef   (:,:,:)
          wk_pl(:,:,:) = rhog_pl(:,:,:) * hdiff_fact_rho * KH_coef_pl(:,:,:)

          call OPRT_diffusion( vtmp2(:,:,:,I_RHOG), vtmp2_pl(:,:,:,I_RHOG), &
                               vtmp (:,:,:,I_RHOG), vtmp_pl (:,:,:,I_RHOG), &
                               wk   (:,:,:),        wk_pl   (:,:,:)         )
       else
          call OPRT_laplacian( vtmp2(:,:,:,I_RHOGE), vtmp2_pl(:,:,:,I_RHOGE), & !--- [OUT]
                               vtmp (:,:,:,I_RHOGE), vtmp_pl (:,:,:,I_RHOGE)  ) !--- [IN]

          call OPRT_laplacian( vtmp2(:,:,:,I_RHOG), vtmp2_pl(:,:,:,I_RHOG), & !--- [OUT]
                               vtmp (:,:,:,I_RHOG), vtmp_pl (:,:,:,I_RHOG)  ) !--- [IN]
       endif
    ...
    if ( gamma_h_lap1 /= 0.D0 ) then
       call OPRT_laplacian( vtmp2    (:,:,:,I_RHOGVX), vtmp2_pl    (:,:,:,I_RHOGVX), & !--- [OUT]
                            vtmp_lap1(:,:,:,I_RHOGVX), vtmp_lap1_pl(:,:,:,I_RHOGVX)  ) !--- [IN]

       call OPRT_laplacian( vtmp2    (:,:,:,I_RHOGVY), vtmp2_pl    (:,:,:,I_RHOGVY), & !--- [OUT]
                            vtmp_lap1(:,:,:,I_RHOGVY), vtmp_lap1_pl(:,:,:,I_RHOGVY)  ) !--- [IN]

       call OPRT_laplacian( vtmp2    (:,:,:,I_RHOGVZ), vtmp2_pl    (:,:,:,I_RHOGVZ), & !--- [OUT]
                            vtmp_lap1(:,:,:,I_RHOGVZ), vtmp_lap1_pl(:,:,:,I_RHOGVZ)  ) !--- [IN]

       call OPRT_laplacian( vtmp2    (:,:,:,I_RHOGW), vtmp2_pl    (:,:,:,I_RHOGW), & !--- [OUT]
                            vtmp_lap1(:,:,:,I_RHOGW), vtmp_lap1_pl(:,:,:,I_RHOGW)  ) !--- [IN]

       wk   (:,:,:) = rhog   (:,:,:) * CNST_CV * KH_coef_lap1   (:,:,:)
       wk_pl(:,:,:) = rhog_pl(:,:,:) * CNST_CV * KH_coef_lap1_pl(:,:,:)

       call OPRT_diffusion( vtmp2    (:,:,:,I_RHOGE), vtmp2_pl    (:,:,:,I_RHOGE), &
                            vtmp_lap1(:,:,:,I_RHOGE), vtmp_lap1_pl(:,:,:,I_RHOGE), &
                            wk       (:,:,:),         wk_pl       (:,:,:)          )
       ...
       call OPRT_diffusion( vtmp2    (:,:,:,I_RHOG), vtmp2_pl    (:,:,:,I_RHOG), &
                            vtmp_lap1(:,:,:,I_RHOG), vtmp_lap1_pl(:,:,:,I_RHOG), &
                            wk       (:,:,:),        wk_pl       (:,:,:)         )
    ...
    !--- Update tendency
    tend(:,:,:,I_RHOG  ) = tend(:,:,:,I_RHOG)                       &
                         - ( vtmp(:,:,:,I_RHOG) + vtmp_lap1(:,:,:,I_RHOG) )

    tend(:,:,:,I_RHOGVX) = tend(:,:,:,I_RHOGVX)                                      &
                         - rhog(:,:,:) * ( vtmp     (:,:,:,I_RHOGVX) * KH_coef     (:,:,:) &
                                         + vtmp_lap1(:,:,:,I_RHOGVX) * KH_coef_lap1(:,:,:) )

    tend(:,:,:,I_RHOGVY) = tend(:,:,:,I_RHOGVY)                                      &
                         - rhog(:,:,:) * ( vtmp     (:,:,:,I_RHOGVY) * KH_coef     (:,:,:) &
                                         + vtmp_lap1(:,:,:,I_RHOGVY) * KH_coef_lap1(:,:,:) )

    tend(:,:,:,I_RHOGVZ) = tend(:,:,:,I_RHOGVZ)                                      &
                         - rhog(:,:,:) * ( vtmp     (:,:,:,I_RHOGVZ) * KH_coef     (:,:,:) &
                                         + vtmp_lap1(:,:,:,I_RHOGVZ) * KH_coef_lap1(:,:,:) )

    tend(:,:,:,I_RHOGW ) = tend(:,:,:,I_RHOGW)                                          &
                         - rhog_h(:,:,:) * ( vtmp     (:,:,:,I_RHOGW) * KH_coef_h     (:,:,:) &
                                           + vtmp_lap1(:,:,:,I_RHOGW) * KH_coef_lap1_h(:,:,:) )

    tend(:,:,:,I_RHOGE ) = tend(:,:,:,I_RHOGE)                        &
                         - ( vtmp(:,:,:,I_RHOGE) + vtmp_lap1(:,:,:,I_RHOGE) )

    tend(:,:,:,I_RHOGETOT) = tend(:,:,:,I_RHOGE)

    if ( ADM_prc_me == ADM_prc_pl ) then
       tend_pl(:,:,:,I_RHOG  ) = tend_pl(:,:,:,I_RHOG)                          &
                               - ( vtmp_pl(:,:,:,I_RHOG) + vtmp_lap1_pl(:,:,:,I_RHOG) )

       tend_pl(:,:,:,I_RHOGVX) = tend_pl(:,:,:,I_RHOGVX)                                            &
                               - rhog_pl(:,:,:) * ( vtmp_pl     (:,:,:,I_RHOGVX) * KH_coef_pl     (:,:,:) &
                                                  + vtmp_lap1_pl(:,:,:,I_RHOGVX) * KH_coef_lap1_pl(:,:,:) )

       tend_pl(:,:,:,I_RHOGVY) = tend_pl(:,:,:,I_RHOGVY)                                            &
                               - rhog_pl(:,:,:) * ( vtmp_pl     (:,:,:,I_RHOGVY) * KH_coef_pl     (:,:,:) &
                                                  + vtmp_lap1_pl(:,:,:,I_RHOGVY) * KH_coef_lap1_pl(:,:,:) )

       tend_pl(:,:,:,I_RHOGVZ) = tend_pl(:,:,:,I_RHOGVZ)                                            &
                               - rhog_pl(:,:,:) * ( vtmp_pl     (:,:,:,I_RHOGVZ) * KH_coef_pl     (:,:,:) &
                                                  + vtmp_lap1_pl(:,:,:,I_RHOGVZ) * KH_coef_lap1_pl(:,:,:) )

       tend_pl(:,:,:,I_RHOGW ) = tend_pl(:,:,:,I_RHOGW)                                                &
                               - rhog_h_pl(:,:,:) * ( vtmp_pl     (:,:,:,I_RHOGW) * KH_coef_h_pl     (:,:,:) &
                                                  + vtmp_lap1_pl(:,:,:,I_RHOGW) * KH_coef_lap1_h_pl(:,:,:) )

       tend_pl(:,:,:,I_RHOGE ) = tend_pl(:,:,:,I_RHOGE)                           &
                               - ( vtmp_pl(:,:,:,I_RHOGE) + vtmp_lap1_pl(:,:,:,I_RHOGE) )

       tend_pl(:,:,:,I_RHOGETOT) = tend_pl(:,:,:,I_RHOGE)
    endif

    call OPRT_horizontalize_vec( tend(:,:,:,I_RHOGVX), tend_pl(:,:,:,I_RHOGVX), & !--- [INOUT]
                                 tend(:,:,:,I_RHOGVY), tend_pl(:,:,:,I_RHOGVY), & !--- [INOUT]
                                 tend(:,:,:,I_RHOGVZ), tend_pl(:,:,:,I_RHOGVZ)  ) !--- [INOUT]
    ...
    if ( TRC_ADV_TYPE == 'DEFAULT' ) then 
       ...
       do l  = 1, ADM_lall
       do k  = ADM_kmin, ADM_kmax
          tendq(:,k,l,nq) = tendq(:,k,l,nq) - ( qtmp(:,k,l,nq) + qtmp_lap1(:,k,l,nq) )
       enddo
       enddo
       enddo
       ...
       if ( ADM_prc_me == ADM_prc_pl ) then
          ...
          do l  = 1, ADM_lall_pl
          do k  = ADM_kmin, ADM_kmax
             tendq_pl(:,k,l,nq) = tendq_pl(:,k,l,nq) - ( qtmp_pl(:,k,l,nq) + qtmp_lap1_pl(:,k,l,nq) )
          enddo
          enddo
          enddo
       endif
  ...
  subroutine numfilter_numerical_vdiff( &
    ...
    do l=1,ADM_lall
       ...
       !
       do k=ADM_kmin, ADM_kmax
          flux(:,k,l,w_id) = (vtmp0(:,k+1,l,w_id) - vtmp0(:,k,l,w_id)) * GRD_rdgz(k) *rho(:,k,l) * VMTR_GSGAM2(:,k,l) * Kv_coef(k)
       enddo

       !--- update tendency
       do k = ADM_kmin, ADM_kmax
          tend(:,k,l,I_RHOG  ) = tend(:,k,l,I_RHOG  ) + ( flux(:,k+1,l,rho_id)-flux(:,k,l,rho_id) ) * GRD_rdgz(k)
          tend(:,k,l,I_RHOGVX) = tend(:,k,l,I_RHOGVX) + ( flux(:,k+1,l,vx_id )-flux(:,k,l,vx_id ) ) * GRD_rdgz(k)
          tend(:,k,l,I_RHOGVY) = tend(:,k,l,I_RHOGVY) + ( flux(:,k+1,l,vy_id )-flux(:,k,l,vy_id ) ) * GRD_rdgz(k)
          tend(:,k,l,I_RHOGVZ) = tend(:,k,l,I_RHOGVZ) + ( flux(:,k+1,l,vz_id )-flux(:,k,l,vz_id ) ) * GRD_rdgz(k)
          tend(:,k,l,I_RHOGE ) = tend(:,k,l,I_RHOGE ) + ( flux(:,k+1,l,tem_id)-flux(:,k,l,tem_id) ) * GRD_rdgz(k)

          tend(:,k,l,I_RHOGETOT) = tend(:,k,l,I_RHOGE )
       enddo

       do k = ADM_kmin+1, ADM_kmax
          tend(:,k,l,I_RHOGW) = tend(:,k,l,I_RHOGW) + ( flux(:,k,l,w_id)-flux(:,k-1,l,w_id) ) * GRD_rdgzh(k)
       enddo

       do nq = 1, TRC_VMAX
       do k  = ADM_kmin, ADM_kmax
          tendq(:,k,l,nq) = tendq(:,k,l,nq) + ( flux(:,k+1,l,nq+vmax)-flux(:,k,l,nq+vmax) ) * GRD_rdgz(k)
       enddo
       enddo
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       ...
       vtmp0_pl(:,:,:,w_id) = w_pl(:,:,:)
       vtmp0_pl(:,:,:,tem_id) = tem_pl(:,:,:) - tem_bs_pl(:,:,:)
       do nq = 1, TRC_VMAX
       ...
       do l=1,ADM_lall
          ...
          !--- update tendency
          do k = ADM_kmin, ADM_kmax
             tend_pl(:,k,l,I_RHOG  ) = tend_pl(:,k,l,I_RHOG  ) + ( flux_pl(:,k+1,l,rho_id)-flux_pl(:,k,l,rho_id) ) * GRD_rdgz(k)
             tend_pl(:,k,l,I_RHOGVX) = tend_pl(:,k,l,I_RHOGVX) + ( flux_pl(:,k+1,l,vx_id )-flux_pl(:,k,l,vx_id ) ) * GRD_rdgz(k)
             tend_pl(:,k,l,I_RHOGVY) = tend_pl(:,k,l,I_RHOGVY) + ( flux_pl(:,k+1,l,vy_id )-flux_pl(:,k,l,vy_id ) ) * GRD_rdgz(k)
             tend_pl(:,k,l,I_RHOGVZ) = tend_pl(:,k,l,I_RHOGVZ) + ( flux_pl(:,k+1,l,vz_id )-flux_pl(:,k,l,vz_id ) ) * GRD_rdgz(k)
             tend_pl(:,k,l,I_RHOGE ) = tend_pl(:,k,l,I_RHOGE ) + ( flux_pl(:,k+1,l,tem_id)-flux_pl(:,k,l,tem_id) ) * GRD_rdgz(k)

             tend_pl(:,k,l,I_RHOGETOT) = tend_pl(:,k,l,I_RHOGE )
          enddo

          do k = ADM_kmin+1, ADM_kmax
             tend_pl(:,k,l,I_RHOGW) = tend_pl(:,k,l,I_RHOGW) + ( flux_pl(:,k,l,w_id)-flux_pl(:,k-1,l,w_id) ) * GRD_rdgzh(k)
          enddo

          do nq = 1, TRC_VMAX
          do k  = ADM_kmin, ADM_kmax
             tendq_pl(:,k,l,nq) = tendq_pl(:,k,l,nq) + ( flux_pl(:,k+1,l,nq+vmax)-flux_pl(:,k,l,nq+vmax) ) * GRD_rdgz(k)
          enddo
          enddo
  ...
  subroutine numfilter_rayleigh_damping( &
    ...
    if ( .NOT. rayleigh_damp_only_w ) then
       ...
       do k = 1, ADM_kall
       do n = 1, ADM_gall
          tend(n,k,l,I_RHOGVX) = tend(n,k,l,I_RHOGVX) &
                               - VMTR_GSGAM2(n,k,l) * rho(n,k,l) * rayleigh(k) * vx(n,k,l)
          tend(n,k,l,I_RHOGVY) = tend(n,k,l,I_RHOGVY) &
                               - VMTR_GSGAM2(n,k,l) * rho(n,k,l) * rayleigh(k) * vy(n,k,l)
          tend(n,k,l,I_RHOGVZ) = tend(n,k,l,I_RHOGVZ) &
                               - VMTR_GSGAM2(n,k,l) * rho(n,k,l) * rayleigh(k) * vz(n,k,l)
       enddo
       enddo
       enddo
       ...
       if ( ADM_prc_me == ADM_prc_pl ) then
          ...
          do k = 1, ADM_kall
          do n = 1, ADM_gall_pl
             tend_pl(n,k,l,I_RHOGVX) = tend_pl(n,k,l,I_RHOGVX) &
                                     - rayleigh(k) * vx_pl(n,k,l) * VMTR_GSGAM2_pl(n,k,l) * rho_pl(n,k,l)
             tend_pl(n,k,l,I_RHOGVY) = tend_pl(n,k,l,I_RHOGVY) &
                                     - rayleigh(k) * vy_pl(n,k,l) * VMTR_GSGAM2_pl(n,k,l) * rho_pl(n,k,l)
             tend_pl(n,k,l,I_RHOGVZ) = tend_pl(n,k,l,I_RHOGVZ) &
                                     - rayleigh(k) * vz_pl(n,k,l) * VMTR_GSGAM2_pl(n,k,l) * rho_pl(n,k,l)
          enddo
          enddo
          enddo
       endif
    ...
    do k = ADM_kmin, ADM_kmax+1
    do n = 1, ADM_gall
       tend(n,k,l,I_RHOGW) = tend(n,k,l,I_RHOGW) &
                           - 0.5D0 * rayleigh_w(k) * w(n,k,l) * ( VMTR_GSGAM2(n,k,  l) * rho(n,k,  l) &
                                                                + VMTR_GSGAM2(n,k-1,l) * rho(n,k-1,l) )
    enddo
    enddo
    enddo
    ...
    if ( ADM_prc_me == ADM_prc_pl ) then
       ...
       do k = ADM_kmin, ADM_kmax+1
       do n = 1, ADM_gall_pl
          tend_pl(n,k,l,I_RHOGW) = tend_pl(n,k,l,I_RHOGW) &
                                 - 0.5D0 * rayleigh_w(k) * w_pl(n,k,l) * ( VMTR_GSGAM2_pl(n,k,  l) * rho_pl(n,k,  l) &
                                                                         + VMTR_GSGAM2_pl(n,k-1,l) * rho_pl(n,k-1,l) )
       enddo
       enddo
       enddo
    endif
