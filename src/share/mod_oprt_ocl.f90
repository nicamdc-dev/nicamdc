...
module mod_oprt
  ...
  !                 06-04-17  add sub[OPRT_divergence] : H.Tomita
  !                 06-08-11  H.Tomita : Implementatio of  miura scheme(2004) 
  !                                      with Thurbuen(1996)'s limiter.
  !                                      Add sub[OPRT_divergence2]
  !                                      sub[OPRT_divergence2_prep] 
  !                                      sub[OPRT_divergence2_all].
  !                 06-08-22  Y.Niwa : divide the rows for the calc. of clap and
  !                                   clap_pl due to the rule of XLF.
  !                 07-01-26  H.Tomita : Optimization of sub[oprt_diffusion].
  !                 07-11-28  T.Mitsui : bugfix in oprt_divergence2, _all
  !                 08-01-24  Y.Niwa : add OPRT_divergence2{_prep,,_all}_rev	
  !                 08-01-30  Y.Niwa : bug fix
  !                 08-04-28  T.Mitsui : bug fix in OPRT_divergence2{_all}_rev	
  !                 09-09-04  H.Taniguchi : bug fix in OPRT_divergence2{_all}_rev
  !                                         Zero clear of wrk[_pl] is needed.
  !                 10-06-08  S.Iga: new grid is implemented (see, string XTMS)
  !                 11-07-22  T.Ohno: The case 'GRD_grid_type' is 'ON_PLANE' is
  !                                   added to wrapper subroutines listed below.
  !                                   1. OPRT_setup
  !                                   2. OPRT_divergence
  !                                   3. OPRT_gradient
  !                                   4. OPRT_laplacian
  ...
  !                                   7. OPRT_vorticity
  !                                   8. OPRT_divdamp
  !                 11-07-28  A.Noda : bug fix
  !                 11-09-27  T.Seiki: merge optimization by RIST and M.Terai 
  !                 11-11-28  Y.Yamada: merge Terai-san timer code into the original code.	
  !                 12-01-17  M.Terai : update optimization(case6) in div2rev
  !                 12-05-01  T.Yamaura : bug fix in div2rev
  !                 12-06-28  M.Terai : Removed wrapper subroutine to invoked the operators directly
  !                                     macro var. _XTMS_ was introduced instead of _XTMS_level_
  !     ------------------------------------------------------------------------
  !
  !-----------------------------------------------------------------------------
 ...
 use mod_adm, only :&
  ...
  ! < for divergence operator >
   real(8),allocatable,save::cdiv(:,:,:,:) !(0:6,gall,lall,1:3)
  !
  real(8),allocatable,private,save::cdiv0_pl(:,:,:)
  real(8),allocatable,private,save::cdiv1_pl(:,:,:)
  real(8),allocatable,private,save::cdiv2_pl(:,:,:)
  real(8),allocatable,private,save::cdiv3_pl(:,:,:)
  real(8),allocatable,private,save::cdiv4_pl(:,:,:)
  real(8),allocatable,private,save::cdiv5_pl(:,:,:)
  !
  ! < for gradient operator >
  real(8),allocatable,private,save::cgrad(:,:,:,:)!(0:6,gall,lall,1:3)
  !
  real(8),allocatable,private,save::cgrad0_pl(:,:,:)
  real(8),allocatable,private,save::cgrad1_pl(:,:,:)
  real(8),allocatable,private,save::cgrad2_pl(:,:,:)
  real(8),allocatable,private,save::cgrad3_pl(:,:,:)
  real(8),allocatable,private,save::cgrad4_pl(:,:,:)
  real(8),allocatable,private,save::cgrad5_pl(:,:,:)
  !
  ! < for laplacian operator >
  real(8),allocatable,private,save::clap0(:,:)
  real(8),allocatable,private,save::clap1(:,:)
  real(8),allocatable,private,save::clap2(:,:)
  real(8),allocatable,private,save::clap3(:,:)
  real(8),allocatable,private,save::clap4(:,:)
  real(8),allocatable,private,save::clap5(:,:)
  real(8),allocatable,private,save::clap6(:,:)
  !
  real(8),allocatable,private,save::clap0_pl(:,:)
  real(8),allocatable,private,save::clap1_pl(:,:)
  real(8),allocatable,private,save::clap2_pl(:,:)
  real(8),allocatable,private,save::clap3_pl(:,:)
  real(8),allocatable,private,save::clap4_pl(:,:)
  real(8),allocatable,private,save::clap5_pl(:,:)
  ...
  !            not needed)
  real(8),allocatable,private,save::cdivN_pl(:,:,:,:) !S.iga100608
  real(8),allocatable,private,save::cgradN_pl(:,:,:,:)!S.iga100608
  real(8),allocatable,private,save::clapN_pl(:,:,:)   !S.iga100608
  real(8),private::tmp1 !S.iga100608
  integer,private::itmp !S.iga100608
  !
  ! < for OPRT_divergence2_prep_rev >  ! Y.Niwa add 080130
  real(8),allocatable,private,save::local_t_var(:,:,:,:,:)
  real(8),allocatable,private,save::local_t_var_pl(:,:,:,:)
  !
  !-----------------------------------------------------------------------------
  !++ Private procedure
  !
...
contains
  ...
  !
  !=============================================================================
  !====================    modules common for both NICAM and XTMS  ============
  !=============================================================================
  !
...
call timer_end(2840)
  ...
  !-----------------------------------------------------------------------------  
  subroutine OPRT_horizontalize_vec( vx, vx_pl, vy, vy_pl, vz, vz_pl )
    use mod_adm, only :   &
         !--- public parameters
         ADM_prc_pl,      &
         ADM_KNONE,       &
...
call timer_sta(2850)
    ...
    fact=1.0d0/GRD_rscale/GRD_rscale
    !
    !nstart=suf(ADM_gmin,ADM_gmin)
    !nend  =suf(ADM_gmax,ADM_gmax)
    !
...
!OCL PARALLEL
      do k=1,ADM_kall
        do n=1,ADM_gall
          prd=(GRD_x(n,ADM_KNONE,l,GRD_XDIR)*vx(n,k,l) &
              +GRD_x(n,ADM_KNONE,l,GRD_YDIR)*vy(n,k,l) &
              +GRD_x(n,ADM_KNONE,l,GRD_ZDIR)*vz(n,k,l))*fact
          vx(n,k,l)=vx(n,k,l)-prd*GRD_x(n,ADM_KNONE,l,GRD_XDIR)
          vy(n,k,l)=vy(n,k,l)-prd*GRD_x(n,ADM_KNONE,l,GRD_YDIR)
          vz(n,k,l)=vz(n,k,l)-prd*GRD_x(n,ADM_KNONE,l,GRD_ZDIR)
        enddo
      enddo
    enddo
    !
...
!OCL PARALLEL
      do n=1,ADM_GALL_PL
        do k=1,ADM_kall
          prd=(GRD_x_pl(n,ADM_KNONE,l,GRD_XDIR)*vx_pl(n,k,l) &
              +GRD_x_pl(n,ADM_KNONE,l,GRD_YDIR)*vy_pl(n,k,l) &
              +GRD_x_pl(n,ADM_KNONE,l,GRD_ZDIR)*vz_pl(n,k,l))*fact
          vx_pl(n,k,l)=vx_pl(n,k,l)-prd*GRD_x_pl(n,ADM_KNONE,l,GRD_XDIR)
          vy_pl(n,k,l)=vy_pl(n,k,l)-prd*GRD_x_pl(n,ADM_KNONE,l,GRD_YDIR)
          vz_pl(n,k,l)=vz_pl(n,k,l)-prd*GRD_x_pl(n,ADM_KNONE,l,GRD_ZDIR)
        enddo
      enddo
      enddo
    endif
...
#else ! end of original modules

  !=============================================================================
  !==================== modules for new grid (Iga 2010) ========================
  !=============================================================================
  subroutine OPRT_setup !XTMS
    use mod_adm, only :   &
         !--- public parameters
         ADM_W,           &
         ADM_prc_pl,      &
    ...
    !
    ! ---- setup coefficient of divergence operator
!2011/02/10 for K by RIST 
!!$    allocate(cdiv0(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv1(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv2(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv3(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv4(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv5(ADM_gall,ADM_lall,1:3))
!!$    allocate(cdiv6(ADM_gall,ADM_lall,1:3))
    allocate(cdiv(0:6,ADM_gall,ADM_lall,1:3))
    !
!    allocate(cdiv0_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
!    allocate(cdiv1_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
...
!    allocate(cdiv4_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
!    allocate(cdiv5_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
    allocate(cdivN_pl(ADM_GALL_PL,ADM_LALL_PL,1:3,0:ADM_XTMS_K))
    !
    !
    allocate(n021((ADM_gall_1d-2)**2))
    !
...
!      n4=ADM_GMIN_PL+4
      do l=1,ADM_LALL_PL
        do m=1,3
          ...
          ! cdivN_pl(:,:,:,0)
          tmp1=0
          do itmp=1,ADM_XTMS_K-1
             tmp1=tmp1+ &
                  gmtr_t_var_pl(ADM_GMIN_PL+itmp-1 ,ADM_KNONE,l,gmtr_t_w1)*gmtr_a_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,md) &
                  +gmtr_t_var_pl(ADM_GMIN_PL+itmp-1 ,ADM_KNONE,l,gmtr_t_w1)*gmtr_a_var_pl(ADM_GMIN_PL+itmp,ADM_KNONE,l,md)
          enddo
          tmp1=tmp1+ &
               gmtr_t_var_pl(ADM_GMIN_PL+ADM_XTMS_K-1 ,ADM_KNONE,l,gmtr_t_w1)&
               *gmtr_a_var_pl(ADM_GMIN_PL+ADM_XTMS_K-1,ADM_KNONE,l,md) &
               +gmtr_t_var_pl(ADM_GMIN_PL+ADM_XTMS_K-1 ,ADM_KNONE,l,gmtr_t_w1)&
               *gmtr_a_var_pl(ADM_GMIN_PL,ADM_KNONE,l,md)
          cdivN_pl(n,l,m,0)=tmp1 *0.5d0*gmtr_p_var_pl(n,ADM_KNONE,l,gmtr_p_rarea)
          !
          !
          ! cdivN_pl(:,:,:,1:ADM_XTMS_K)
          do itmp=1,ADM_XTMS_K
             n0=ADM_GMIN_PL+itmp-1
             n1=ADM_GMIN_PL+itmp
             n4=ADM_GMIN_PL+itmp-2
             if (itmp==1) then
                n4=n4+ADM_XTMS_K
             elseif (itmp==ADM_XTMS_K) then
                n1=n1-ADM_XTMS_K
             endif
             cdivN_pl(n,l,m,itmp)=(                                                                         &
                  +gmtr_t_var_pl(n0,ADM_KNONE,l,gmtr_t_w2)*gmtr_a_var_pl(n0,ADM_KNONE,l,md) &
...
!                          )*0.5d0*gmtr_p_var_pl(n,ADM_KNONE,l,gmtr_p_rarea)
    ...
    ! ---- setup coefficient of gradient operator
!2011/02/10 for K by RIST 
!!$    allocate(cgrad0(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad1(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad2(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad3(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad4(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad5(ADM_gall,ADM_lall,1:3))
!!$    allocate(cgrad6(ADM_gall,ADM_lall,1:3))
    allocate(cgrad(0:6,ADM_gall,ADM_lall,1:3))
    !
!    allocate(cgrad0_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
!    allocate(cgrad1_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
...
!    allocate(cgrad4_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
!    allocate(cgrad5_pl(ADM_GALL_PL,ADM_LALL_PL,1:3))
    allocate(cgradN_pl(ADM_GALL_PL,ADM_LALL_PL,1:3,0:ADM_XTMS_K))
...
!      n4=ADM_GMIN_PL+4
      do l=1,ADM_LALL_PL
        do m=1,3
          ...
          do itmp=1,ADM_XTMS_K-1
             tmp1=tmp1&
                  +2*(gmtr_t_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,gmtr_t_w1)-1.0d0) &
                  *gmtr_a_var_pl(ADM_GMIN_PL+itmp,ADM_KNONE,l,md)                  
          enddo
          tmp1=tmp1&
                  +2*(gmtr_t_var_pl(ADM_GMIN_PL+ADM_XTMS_K-1,ADM_KNONE,l,gmtr_t_w1)-1.0d0) &
                  *gmtr_a_var_pl(ADM_GMIN_PL,ADM_KNONE,l,md)
          cgradN_pl(n,l,m,0)=   tmp1 *gmtr_p_var_pl(n,ADM_KNONE,l,gmtr_p_rarea)*0.5d0                                           
          !
          !
          !n0-n4
          !
...
!    allocate(clap4_pl(ADM_GALL_PL,ADM_LALL_PL))
!    allocate(clap5_pl(ADM_GALL_PL,ADM_LALL_PL))
    allocate(clapN_pl(ADM_GALL_PL,ADM_LALL_PL,0:ADM_XTMS_K))
      ...
      do l=1,ADM_LALL_PL
         ...
         do itmp=1,ADM_XTMS_K
            ...
            tmp1=tmp1&
                       ...
                       -1*gmtr_a_var_pl(n1,k0,l,tz1)*gmtr_t_var_pl(n0,k0,l,a0)*gmtr_a_var_pl(n0,k0,l,hz1)
         enddo
         clapN_pl(n,l,0)=   tmp1 * gmtr_p_var_pl(n,k0,l,gmtr_p_rarea)/12.0d0
         !
         !
         !n0-n4
         !
...
!!$                      )*gmtr_p_var_pl(n,k0,l,gmtr_p_rarea)/12.0d0
  ...
  end subroutine OPRT_setup !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence(      & !XTMS
       scl, scl_pl,                &
       vx, vx_pl,                  &
       vy, vy_pl,                  &
       vz, vz_pl,                  &
...
!      n4=ADM_GMIN_PL+4
      do l=1,ADM_LALL_PL
        do k=1,ADM_kall
           ! iga changed
           scl_pl(n,k,l)=0
           scl_pl(n,k,l)=scl_pl(n,k,l)+cdivN_pl(n,l,1,0)*vx_pl(n ,k,l)
           scl_pl(n,k,l)=scl_pl(n,k,l)+cdivN_pl(n,l,2,0)*vy_pl(n ,k,l)
           scl_pl(n,k,l)=scl_pl(n,k,l)+cdivN_pl(n,l,3,0)*vz_pl(n ,k,l)
           do itmp=1,ADM_XTMS_K
              scl_pl(n,k,l)=scl_pl(n,k,l) &
                   +cdivN_pl(n,l,1,itmp)*vx_pl(ADM_GMIN_PL+itmp-1  ,k,l)  &
                   +cdivN_pl(n,l,2,itmp)*vy_pl(ADM_GMIN_PL+itmp-1  ,k,l)  &
                   +cdivN_pl(n,l,3,itmp)*vz_pl(ADM_GMIN_PL+itmp-1  ,k,l)
           enddo
...
!!$                        )*fact
  ...
  end subroutine OPRT_divergence !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_gradient(        & !XTMS
       vx, vx_pl,                  &
       vy, vy_pl,                  &
       vz, vz_pl,                  &
       scl, scl_pl,                &
...
!      n4=ADM_GMIN_PL+4
      do l=1,ADM_LALL_PL
        do k=1,ADM_kall
           ...
           !iga start
           !
           vx_pl(n,k,l)=cgradN_pl(n,l,1,0)*scl_pl(n ,k,l)
           vy_pl(n,k,l)=cgradN_pl(n,l,2,0)*scl_pl(n ,k,l)
           vz_pl(n,k,l)=cgradN_pl(n,l,3,0)*scl_pl(n ,k,l)
           do itmp=1,ADM_XTMS_K
              vx_pl(n,k,l)=vx_pl(n,k,l)+ cgradN_pl(n,l,1,itmp)*scl_pl(ADM_GMIN_PL+itmp-1,k,l)
              vy_pl(n,k,l)=vy_pl(n,k,l)+ cgradN_pl(n,l,2,itmp)*scl_pl(ADM_GMIN_PL+itmp-1,k,l)
              vz_pl(n,k,l)=vz_pl(n,k,l)+ cgradN_pl(n,l,3,itmp)*scl_pl(ADM_GMIN_PL+itmp-1,k,l)
           enddo
...
!!$                       )*fact
    ...
    !
  end subroutine OPRT_gradient !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_laplacian(dscl,dscl_pl,scl, scl_pl,mfact) !XTMS
    use mod_adm, only :   &
         !--- public parameters
         ADM_prc_pl,      &
         ADM_TI,          &
...
!      n4=ADM_GMIN_PL+4
      do l=1,ADM_LALL_PL
        do k=1,ADM_kall
           ...
           !
           !iga start
           dscl_pl(n,k,l)=clapN_pl(n,l,0)*scl_pl(n ,k,l)
           do itmp=1,ADM_XTMS_K
              dscl_pl(n,k,l)=dscl_pl(n,k,l)+clapN_pl(n,l,itmp)*scl_pl(ADM_GMIN_PL+itmp-1,k,l)
           enddo
...
!!$                         )*fact
    ...
    !
  end subroutine OPRT_laplacian !XTMS
  !-----------------------------------------------------------------------------  
  subroutine OPRT_diffusion( & !XTMS
       dscl, dscl_pl,        &
       scl,scl_pl,           &
       kh, kh_pl,            &
       mfact )
    ...
    do l=1,ADM_lall
       ...
       !Do k=ADM_kmin,ADM_kmax
          ...
          do n = nstart,nend
             !
             smean = (scl(n,k,l)+scl(n+1,k,l)+scl(n+1+ADM_gall_1d,k,l))/3.0D0
             u1=-GMTR_T_var(n,ADM_KNONE,l,ADM_TI,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n,k,l)+scl(n+1,k,l))-smean)
             u2=-GMTR_T_var(n,ADM_KNONE,l,ADM_TI,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n+1,k,l)+scl(n+1+ADM_gall_1d,k,l))-smean)
             u3=+GMTR_T_var(n,ADM_KNONE,l,ADM_TI,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n+1+ADM_gall_1d,k,l)+scl(n,k,l))-smean)
             vxt(n,k,l,ADM_TI)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_TNX)&
                  +u2*GMTR_A_var(n+1,ADM_KNONE,l,ADM_AJ,GMTR_A_TNX)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNX)
             vyt(n,k,l,ADM_TI)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_TNY)&
                  +u2*GMTR_A_var(n+1,ADM_KNONE,l,ADM_AJ,GMTR_A_TNY)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNY)
             vzt(n,k,l,ADM_TI)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_TNZ)&
                  +u2*GMTR_A_var(n+1,ADM_KNONE,l,ADM_AJ,GMTR_A_TNZ)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNZ)
             !
             smean = (scl(n,k,l)+scl(n+1+ADM_gall_1d,k,l)+scl(n+ADM_gall_1d,k,l))/3.0D0
             u1=-GMTR_T_var(n,ADM_KNONE,l,ADM_TJ,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n,k,l)+scl(n+1+ADM_gall_1d,k,l))-smean)
             u2=+GMTR_T_var(n,ADM_KNONE,l,ADM_TJ,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n+1+ADM_gall_1d,k,l)+scl(n+ADM_gall_1d,k,l))-smean)
             u3=+GMTR_T_var(n,ADM_KNONE,l,ADM_TJ,GMTR_T_RAREA)&
                  *(0.5D0*(scl(n+ADM_gall_1d,k,l)+scl(n,k,l))-smean)
             vxt(n,k,l,ADM_TJ)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNX)&
                  +u2*GMTR_A_var(n+ADM_gall_1d,ADM_KNONE,l,ADM_AI,GMTR_A_TNX)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_TNX)
             vyt(n,k,l,ADM_TJ)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNY)&
                  +u2*GMTR_A_var(n+ADM_gall_1d,ADM_KNONE,l,ADM_AI,GMTR_A_TNY)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_TNY)
             vzt(n,k,l,ADM_TJ)&
                  =u1*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_TNZ)&
                  +u2*GMTR_A_var(n+ADM_gall_1d,ADM_KNONE,l,ADM_AI,GMTR_A_TNZ)&
                  +u3*GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_TNZ)
             !
          end do
          !
          if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
    ...
    do l=1,ADM_lall
       ...
       do k= 1, ADM_kall
          ...
          do n = nstart,nend
             flux(n,ADM_AI)&
                  =((vxt(n-ADM_gall_1d,k,l,ADM_TJ)+vxt(n,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_HNX)&
                  +(vyt(n-ADM_gall_1d,k,l,ADM_TJ)+vyt(n,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_HNY)&
                  +(vzt(n-ADM_gall_1d,k,l,ADM_TJ)+vzt(n,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_HNZ))*0.5D0
             flux(n,ADM_AI) = flux(n,ADM_AI)&
                  * (kh(n,k,l)+kh(n+1,k,l))*0.5D0
          end do
          nstart = suf(ADM_gmin-1,ADM_gmin-1)
          nend   = suf(ADM_gmax  ,ADM_gmax  )
          do n = nstart,nend
             flux(n,ADM_AIJ)&
                  =((vxt(n,k,l,ADM_TI)+vxt(n,k,l,ADM_TJ))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_HNX)&
                  +(vyt(n,k,l,ADM_TI)+vyt(n,k,l,ADM_TJ))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_HNY)&
                  +(vzt(n,k,l,ADM_TI)+vzt(n,k,l,ADM_TJ))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_HNZ))*0.5D0
             flux(n,ADM_AIJ) = flux(n,ADM_AIJ)&
                  * (kh(n,k,l)+kh(n+ADM_gall_1d+1,k,l))*0.5D0
          end do
          nstart = suf(ADM_gmin  ,ADM_gmin-1)
          nend   = suf(ADM_gmax  ,ADM_gmax  )
          do n = nstart,nend
             flux(n,ADM_AJ)&
                  =((vxt(n,k,l,ADM_TJ)+vxt(n-1,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_HNX)&
                  +(vyt(n,k,l,ADM_TJ)+vyt(n-1,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_HNY)&
                  +(vzt(n,k,l,ADM_TJ)+vzt(n-1,k,l,ADM_TI))&
                  *GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_HNZ))*0.5D0
             flux(n,ADM_AJ) = flux(n,ADM_AJ)&
                  * (kh(n,k,l)+kh(n+ADM_gall_1d,k,l))*0.5D0
          end do
          !
          nstart = suf(ADM_gmin  ,ADM_gmin  )
          nend   = suf(ADM_gmax  ,ADM_gmax  )
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       ...
       do k= 1, ADM_kall
          do l=1,ADM_LALL_PL
             ...
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)
             enddo
             dscl_pl(ADM_GSLF_PL,k,l)=tmp1* GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA)*fact
...
!!$                  * fact
  ...
  end subroutine OPRT_diffusion !XTMS
  !-----------------------------------------------------------------------------  
  subroutine OPRT_horizontalize_vec( vx, vx_pl, vy, vy_pl, vz, vz_pl ) !XTMS
    use mod_adm, only :   &
         !--- public parameters
         ADM_prc_pl,      &
         ADM_KNONE,       &
  ...
  end subroutine OPRT_horizontalize_vec !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_vorticity(       & !XTMS
       scl, scl_pl,                &
       vx, vx_pl,                  &
       vy, vy_pl,                  &
       vz, vz_pl,                  &
...
!                  * fact
    ...
    !
  end subroutine OPRT_vorticity !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divdamp(       & !XTMS
       grdx, grdx_pl,            &
       grdy, grdy_pl,            &
       grdz, grdz_pl,            &
       vx, vx_pl,                &
    ...
    do l=1,ADM_lall
       ...
       do k=ADM_kmin,ADM_kmax
          ...
          if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
             i=ADM_gmin
             j=ADM_gmin
             dp1=flux(suf(i,j),ADM_AI )
             dp2=flux(suf(i,j),ADM_AIJ)
             dp3=flux(suf(i,j),ADM_AJ )
             dp4=flux(suf(i-1,j),ADM_AI )
             dp5=flux(suf(i-1,j-1),ADM_AIJ)
             !
             grdx(suf(i,j),k,l)=(                                     &
                  +dp1*GMTR_A_var(suf(i,j),ADM_KNONE,l,ADM_AI ,GMTR_A_HNX) &
                  +dp2*GMTR_A_var(suf(i,j),ADM_KNONE,l,ADM_AIJ,GMTR_A_HNX) &
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do k=ADM_kmin,ADM_kmax
          do l=1,ADM_LALL_PL
             ...
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)+GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNX)
             enddo
             grdx_pl(ADM_GSLF_PL,k,l)=(tmp1 &
                  ) * GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA)  &
                  * fact
             !
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)+GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNY)
             enddo
             grdy_pl(ADM_GSLF_PL,k,l)=(tmp1 &
                  ) * GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA) &
                  * fact
             !
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)+GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNZ)
             enddo
             grdz_pl(ADM_GSLF_PL,k,l)=(tmp1 &
                  ) * GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA) &
                  * fact
             !
...
!!$                  * fact
    ...
    !
  end subroutine OPRT_divdamp !XTMS
  !-----------------------------------------------------------------------------
  subroutine intpl_p2t( & !XTMS
       vt, vt_pl,       &
       v, v_pl          &
       )
    !
    ...
    !
  end subroutine intpl_p2t !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_gradient_nobase( & !XTMS
       vx, vx_pl,                  &
       vy, vy_pl,                  &
       vz, vz_pl,                  &
       scl, scl_pl,                &
       ...
       do k=1, ADM_kall
          do l=1,ADM_LALL_PL
             ...
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)*GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNX)
             enddo
             vx_pl(ADM_GSLF_PL,k,l)=tmp1*GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA)*fact
             !
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)*GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNY)
             enddo
             vy_pl(ADM_GSLF_PL,k,l)=tmp1*GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA)*fact
             !
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flux_pl(ADM_GMIN_PL+itmp-1)*GMTR_A_var_pl(ADM_GMIN_PL+itmp-1,ADM_KNONE,l,GMTR_A_HNZ)
             enddo
             vz_pl(ADM_GSLF_PL,k,l)=tmp1*GMTR_P_var_pl(ADM_GSLF_PL,ADM_KNONE,l,GMTR_P_RAREA)*fact
             !
!!$             dp1=flux_pl(ADM_GMIN_PL  )
!!$             dp2=flux_pl(ADM_GMIN_PL+1)
!!$             dp3=flux_pl(ADM_GMIN_PL+2)
...
!!$             !
    ...
    !
  end subroutine OPRT_gradient_nobase !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_prep(& !XTMS
       c,  c_pl,                   &
       cp, cp_pl,                  &
       vx, vx_pl,                  &
       vy, vy_pl,                  &
    ...
    !
  end subroutine OPRT_divergence2_prep !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2(     & !XTMS
       scl, scl_pl,                &
       rho, rho_pl,                &
       c,  c_pl,                   &
       cp, cp_pl,                  &
    ...
    do l=1,ADM_lall
       ...
       do k = 1, ADM_kall
          ...
          do n=nstart,nend
             if(c(n  ,k,l,1)<=0.0D0) then
                rhoi_min(n,k,l,1) = min(rho(n,k,l),rho(n+1,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-ADM_gall_1d,k,l))
                rhoi_max(n,k,l,1) = max(rho(n,k,l),rho(n+1,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-ADM_gall_1d,k,l))
             else
                rhoi_min(n+1,k,l,4) = min(rho(n,k,l),rho(n+1,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-ADM_gall_1d,k,l))
                rhoi_max(n+1,k,l,4) = max(rho(n,k,l),rho(n+1,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-ADM_gall_1d,k,l))
             end if
          end do
          !
          nstart = suf(ADM_gmin-1,ADM_gmin-1)
          ...
          do n=nstart,nend
             !
             if(c(n  ,k,l,2)<=0.0D0) then
                rhoi_min(n,k,l,2) = min(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1,k,l),rho(n+ADM_gall_1d,k,l))
                rhoi_max(n,k,l,2) = max(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1,k,l),rho(n+ADM_gall_1d,k,l))
             else
                rhoi_min(n+1+ADM_gall_1d,k,l,5) = min(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1,k,l),rho(n+ADM_gall_1d,k,l))
                rhoi_max(n+1+ADM_gall_1d,k,l,5) = max(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1,k,l),rho(n+ADM_gall_1d,k,l))
             end if
             !
          end do
          !
          ...
          do n=nstart,nend
             if(c(n  ,k,l,3)<=0.0D0) then
                rhoi_min(n,k,l,3) = min(rho(n,k,l),rho(n+ADM_gall_1d,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-1,k,l))
                rhoi_max(n,k,l,3) = max(rho(n,k,l),rho(n+ADM_gall_1d,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-1,k,l))
             else
                rhoi_min(n+ADM_gall_1d,k,l,6) = min(rho(n,k,l),rho(n+ADM_gall_1d,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-1,k,l))
                rhoi_max(n+ADM_gall_1d,k,l,6) = max(rho(n,k,l),rho(n+ADM_gall_1d,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n-1,k,l))
             end if
             !
          end do
          if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
             n = suf(ADM_gmin-1,ADM_gmin-1)
             if(c(n  ,k,l,2)<=0.0D0) then
                rhoi_min(n,k,l,2) = min(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1+1+ADM_gall_1d,k,l),rho(n+ADM_gall_1d,k,l))
                rhoi_max(n,k,l,2) = max(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),rho(n+1+1+ADM_gall_1d,k,l),rho(n+ADM_gall_1d,k,l))
             else
                rhoi_min(n+1+ADM_gall_1d,k,l,5) = min(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),&
                     rho(n+1+1+ADM_gall_1d,k,l),rho(n+ADM_gall_1d,k,l))
                rhoi_max(n+1+ADM_gall_1d,k,l,5) = max(rho(n,k,l),rho(n+1+ADM_gall_1d,k,l),&
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do l=1,ADM_LALL_PL
          do k = 1, ADM_kall
             ...
             do n=ADM_GMIN_PL,ADM_GMAX_PL
                if(c_pl(n,k,l)<=0.0D0) then
                   rhoi_min_pl(n,k,l,1) = min(rho_pl(ADM_GSLF_PL,k,l),rho_pl(n,k,l),rho_pl(nm1(n),k,l),rho_pl(np1(n),k,l))
                   rhoi_max_pl(n,k,l,1) = max(rho_pl(ADM_GSLF_PL,k,l),rho_pl(n,k,l),rho_pl(nm1(n),k,l),rho_pl(np1(n),k,l))
                else
                   rhoi_min_pl(n,k,l,2) = min(rho_pl(ADM_GSLF_PL,k,l),rho_pl(n,k,l),rho_pl(nm1(n),k,l),rho_pl(np1(n),k,l))
                   rhoi_max_pl(n,k,l,2) = max(rho_pl(ADM_GSLF_PL,k,l),rho_pl(n,k,l),rho_pl(nm1(n),k,l),rho_pl(np1(n),k,l))
                end if
             end do
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do l=1,ADM_LALL_PL
          do k = 1, ADM_kall
             rho_m1_k_min_pl &
                  = min(rhoi_min_pl(ADM_GMIN_PL,k,l,1),rhoi_min_pl(ADM_GMIN_PL+1,k,l,1),&
                  rhoi_min_pl(ADM_GMIN_PL+2,k,l,1),rhoi_min_pl(ADM_GMIN_PL+3,k,l,1),rhoi_min_pl(ADM_GMIN_PL+4,k,l,1))
             if(rho_m1_k_min_pl== CNST_MAX_REAL) rho_m1_k_min_pl=rho_pl(ADM_GSLF_PL,k,l)
             rho_m1_k_max_pl &
                  = max(rhoi_min_pl(ADM_GMIN_PL,k,l,1),rhoi_min_pl(ADM_GMIN_PL+1,k,l,1),&
                  rhoi_min_pl(ADM_GMIN_PL+2,k,l,1),rhoi_min_pl(ADM_GMIN_PL+3,k,l,1),rhoi_min_pl(ADM_GMIN_PL+4,k,l,1))
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do l=1,ADM_LALL_PL
          do k = 1, ADM_kall
             ...
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+  c_pl(ADM_GMIN_PL+itmp-1 ,k,l)*rhoa_pl(ADM_GMIN_PL+itmp-1  ,k,l)
             enddo
             scl_pl(ADM_GSLF_PL,k,l)=tmp1* fact/dt
!!$             scl_pl(ADM_GSLF_PL,k,l)=(&
!!$                  +c_pl(ADM_GMIN_PL  ,k,l)*rhoa_pl(ADM_GMIN_PL  ,k,l)  &
!!$                  +c_pl(ADM_GMIN_PL+1,k,l)*rhoa_pl(ADM_GMIN_PL+1,k,l)&
!!$                  +c_pl(ADM_GMIN_PL+2,k,l)*rhoa_pl(ADM_GMIN_PL+2,k,l)&
...
!!$                  ) * fact/dt
    ...
    !
  end subroutine OPRT_divergence2 !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_all( & !XTMS
       scl, scl_pl,                &
       rho, rho_pl,                &
       c,  c_pl,                   &
       cp, cp_pl,                  &
       ...
       if(ADM_prc_me==ADM_prc_pl) then
          do l=1,ADM_LALL_PL
             do k = 1, ADM_kall
                ...
                c_in_sum_pl=0
                do itmp=1,ADM_XTMS_K
                   c_in_sum_pl=c_in_sum_pl+(0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1,k,l)))*c_pl(ADM_GMIN_PL+itmp-1,k,l)
                enddo
                c_out_sum_pl=0
                do itmp=1,ADM_XTMS_K
                   c_out_sum_pl=c_out_sum_pl+(0.5D0+sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1,k,l)))*c_pl(ADM_GMIN_PL+itmp-1,k,l)
                enddo
                c_rhoin_sum_max_pl=0
                do itmp=1,ADM_XTMS_K
                   c_rhoin_sum_max_pl=c_rhoin_sum_max_pl+&
                        (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1,k,l)))&
                        *(c_pl(ADM_GMIN_PL+itmp-1,k,l)*rhoi_max_pl(ADM_GMIN_PL+itmp-1,k,l,1,nq))
                enddo
                c_rhoin_sum_min_pl=0
                do itmp=1,ADM_XTMS_K
                   c_rhoin_sum_min_pl=c_rhoin_sum_min_pl+&
                        (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1  ,k,l)))&
                        *(c_pl(ADM_GMIN_PL+itmp-1  ,k,l)*rhoi_min_pl(ADM_GMIN_PL+itmp-1  ,k,l,1,nq))
                enddo
          ...
          do l=1,ADM_LALL_PL
             do k = 1, ADM_kall
                ...
                do itmp=1,ADM_XTMS_K
                   tmp1=tmp1+  c_pl(ADM_GMIN_PL+itmp-1 ,k,l)*rhoa_pl(ADM_GMIN_PL+itmp-1  ,k,l)
                enddo
                scl_pl(ADM_GSLF_PL,k,l,nq)=tmp1* fact/dt
...
!!$                     ) * fact/dt
  ...
  end subroutine OPRT_divergence2_all !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_prep_rev( & !XTMS
       flx_h,   flx_h_pl,  &
       cp,      cp_pl,     &
       rhovx,  rhovx_pl, &
       rhovy,  rhovy_pl, &
    ...
    implicit none
    !
    real(8), intent(out) :: flx_h(ADM_gall,ADM_kall,ADM_lall,6)
    real(8), intent(out) :: flx_h_pl(ADM_GALL_PL,ADM_kall,ADM_LALL_PL)
    real(8), intent(out) :: cp(ADM_gall,ADM_kall,ADM_lall,ADM_AI:ADM_AJ,GRD_XDIR:GRD_ZDIR)
    real(8), intent(out) :: cp_pl(ADM_GALL_PL,ADM_kall,ADM_LALL_PL,GRD_XDIR:GRD_ZDIR)
    real(8), intent(in)  :: rhovx(ADM_gall,ADM_kall,ADM_lall)
...
!!$                     *rho_pl(np1(n),k,l)
       ...
       do k = 1, ADM_kall
          ...
          do n = nstart,nend
             ...
             ccc = &
                  ...
                  +rhovya(n,ADM_AI)*GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_HNY)&
                  +rhovza(n,ADM_AI)*GMTR_A_var(n,ADM_KNONE,l,ADM_AI,GMTR_A_HNZ))
             flx_h(n  ,k,l,1) = ccc*dt*GMTR_P_var(n,ADM_KNONE,l,GMTR_P_RAREA)
             flx_h(n+1,k,l,4) =-ccc*dt*GMTR_P_var(n+1,ADM_KNONE,l,GMTR_P_RAREA)
             !
          end do
          !
          !
          ...
          do n = nstart,nend
             ...
             ccc = &
                  ...
                  +rhovya(n,ADM_AIJ)*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_HNY)&
                  +rhovza(n,ADM_AIJ)*GMTR_A_var(n,ADM_KNONE,l,ADM_AIJ,GMTR_A_HNZ))
             flx_h(n              ,k,l,2) = ccc*dt*GMTR_P_var(n,ADM_KNONE,l,GMTR_P_RAREA)
             flx_h(n+1+ADM_gall_1d,k,l,5) =-ccc*dt*GMTR_P_var(n+1+ADM_gall_1d,ADM_KNONE,l,GMTR_P_RAREA)
             !
          end do
          nstart = suf(ADM_gmin  ,ADM_gmin-1)
          nend   = suf(ADM_gmax  ,ADM_gmax  )
          do n = nstart,nend
             ...
             ccc = &
                  ...
                  +rhovya(n,ADM_AJ)*GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_HNY)&
                  +rhovza(n,ADM_AJ)*GMTR_A_var(n,ADM_KNONE,l,ADM_AJ,GMTR_A_HNZ))
             flx_h(n            ,k,l,3) = ccc*dt*GMTR_P_var(n,ADM_KNONE,l,GMTR_P_RAREA)
             flx_h(n+ADM_gall_1d,k,l,6) =-ccc*dt*GMTR_P_var(n+ADM_gall_1d,ADM_KNONE,l,GMTR_P_RAREA)
          end do
          !
          if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
             flx_h(suf(ADM_gmin,ADM_gmin),k,l,6) = 0.0D0
          end if
       end do
    end do
    !
    ...
    !
    return
  end subroutine OPRT_divergence2_prep_rev !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_rev( & !XTMS
       scl,   scl_pl,       &
       s,     s_pl,         &
       flx_h, flx_h_pl,     &
       c,     c_pl,         &
    ...
    real(8)  :: sa_p,sa_m
    !
    real(8)  :: s_in_min(ADM_gall,ADM_kall,ADM_lall,6)
    real(8)  :: s_in_min_pl(ADM_GALL_PL,ADM_kall,ADM_LALL_PL,2)
    real(8)  :: s_in_max(ADM_gall,ADM_kall,ADM_lall,6)
    real(8)  :: s_in_max_pl(ADM_GALL_PL,ADM_kall,ADM_LALL_PL,2)
    !
    real(8)  :: s_m1_k_min(ADM_gall)
    real(8)  :: s_m1_k_min_pl
...
!    logical :: NON_NEG=.false.
    ...
    wrk(:,:,:,s_out_k_min) = s(:,:,:)
    wrk(:,:,:,s_out_k_max) = s(:,:,:)
    wrk_pl(:,:,:,s_out_k_min) = s_pl(:,:,:)
    wrk_pl(:,:,:,s_out_k_max) = s_pl(:,:,:)
    !
    if(present(mfact)) then
       fact=mfact/dt
    else
    ...
    do l=1,ADM_lall
       ...
       do k = 1, ADM_kall
          ...
          nstart = suf(ADM_gmin-1,ADM_gmin-1)
          nend   = suf(ADM_gmax  ,ADM_gmax  )
          s_in_min(:,k,l,:) = CNST_MAX_REAL
          s_in_max(:,k,l,:) =-CNST_MAX_REAL
          !
          nstart = suf(ADM_gmin-1,ADM_gmin  )
          nend   = suf(ADM_gmax,  ADM_gmax  )
          do n=nstart,nend
             if(c(n  ,k,l,1)<=0.0D0) then
                s_in_min(n,k,l,1) = min(s(n,k,l),s(n+1,k,l),s(n+1+ADM_gall_1d,k,l),s(n-ADM_gall_1d,k,l))
                s_in_max(n,k,l,1) = max(s(n,k,l),s(n+1,k,l),s(n+1+ADM_gall_1d,k,l),s(n-ADM_gall_1d,k,l))
             else
                s_in_min(n+1,k,l,4) = min(s(n,k,l),s(n+1,k,l),s(n+1+ADM_gall_1d,k,l),s(n-ADM_gall_1d,k,l))
                s_in_max(n+1,k,l,4) = max(s(n,k,l),s(n+1,k,l),s(n+1+ADM_gall_1d,k,l),s(n-ADM_gall_1d,k,l))
             end if
          end do
          !
          nstart = suf(ADM_gmin-1,ADM_gmin-1)
          ...
          do n=nstart,nend
             !
             if(c(n  ,k,l,2)<=0.0D0) then
                s_in_min(n,k,l,2) = min(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1,k,l),s(n+ADM_gall_1d,k,l))
                s_in_max(n,k,l,2) = max(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1,k,l),s(n+ADM_gall_1d,k,l))
             else
                s_in_min(n+1+ADM_gall_1d,k,l,5) = min(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1,k,l),s(n+ADM_gall_1d,k,l))
                s_in_max(n+1+ADM_gall_1d,k,l,5) = max(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1,k,l),s(n+ADM_gall_1d,k,l))
             end if
             !
          end do
          !
          ...
          do n=nstart,nend
             if(c(n  ,k,l,3)<=0.0D0) then
                s_in_min(n,k,l,3) = min(s(n,k,l),s(n+ADM_gall_1d,k,l),s(n+1+ADM_gall_1d,k,l),s(n-1,k,l))
                s_in_max(n,k,l,3) = max(s(n,k,l),s(n+ADM_gall_1d,k,l),s(n+1+ADM_gall_1d,k,l),s(n-1,k,l))
             else
                s_in_min(n+ADM_gall_1d,k,l,6) = min(s(n,k,l),s(n+ADM_gall_1d,k,l),s(n+1+ADM_gall_1d,k,l),s(n-1,k,l))
                s_in_max(n+ADM_gall_1d,k,l,6) = max(s(n,k,l),s(n+ADM_gall_1d,k,l),s(n+1+ADM_gall_1d,k,l),s(n-1,k,l))
             end if
             !
          end do
          if(ADM_rgn_vnum(ADM_W,rgnid)==3) then
             n = suf(ADM_gmin-1,ADM_gmin-1)
             if(c(n  ,k,l,2)<=0.0D0) then
                s_in_min(n,k,l,2) = min(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1+1+ADM_gall_1d,k,l),s(n+ADM_gall_1d,k,l))
                s_in_max(n,k,l,2) = max(s(n,k,l),s(n+1+ADM_gall_1d,k,l),s(n+1+1+ADM_gall_1d,k,l),s(n+ADM_gall_1d,k,l))
             else
                s_in_min(n+1+ADM_gall_1d,k,l,5) = min(s(n,k,l),s(n+1+ADM_gall_1d,k,l),&
                     s(n+1+1+ADM_gall_1d,k,l),s(n+ADM_gall_1d,k,l))
                s_in_max(n+1+ADM_gall_1d,k,l,5) = max(s(n,k,l),s(n+1+ADM_gall_1d,k,l),&
                     s(n+1+1+ADM_gall_1d,k,l),s(n+ADM_gall_1d,k,l))
             end if
          end if
          !
    ...
    do l=1,ADM_lall
       ...
       do k = 1, ADM_kall
          ...
          nend   = suf(ADM_gmax+1,ADM_gmax+1)
          do n = nstart,nend
             s_m1_k_min(n) = min(s_in_min(n,k,l,1),s_in_min(n,k,l,2),s_in_min(n,k,l,3),&
                  s_in_min(n,k,l,4),s_in_min(n,k,l,5),s_in_min(n,k,l,6))
             if(s_m1_k_min(n)==CNST_MAX_REAL) s_m1_k_min(n) = s(n,k,l)
             !        s_m1_k_min(n) = max(SMALL_ZERO,s_m1_k_min(n))
             s_m1_k_max(n) = max(s_in_max(n,k,l,1),s_in_max(n,k,l,2),s_in_max(n,k,l,3),&
                  s_in_max(n,k,l,4),s_in_max(n,k,l,5),s_in_max(n,k,l,6))
             if(s_m1_k_max(n)==-CNST_MAX_REAL) s_m1_k_max(n) = s(n,k,l)
             !
          end do
          !
          nstart = suf(ADM_gmin,  ADM_gmin  )
          ...
          do n = nstart,nend
             ...
             !
             c_qin_sum_max(n) &
                  = (0.5D0-sign(0.5D0,c(n,k,l,1)))*(c(n,k,l,1)*s_in_max(n,k,l,1))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,2)))*(c(n,k,l,2)*s_in_max(n,k,l,2))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,3)))*(c(n,k,l,3)*s_in_max(n,k,l,3))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,4)))*(c(n,k,l,4)*s_in_max(n,k,l,4))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,5)))*(c(n,k,l,5)*s_in_max(n,k,l,5))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,6)))*(c(n,k,l,6)*s_in_max(n,k,l,6))

             c_qin_sum_min(n) &
                  = (0.5D0-sign(0.5D0,c(n,k,l,1)))*(c(n,k,l,1)*s_in_min(n,k,l,1))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,2)))*(c(n,k,l,2)*s_in_min(n,k,l,2))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,3)))*(c(n,k,l,3)*s_in_min(n,k,l,3))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,4)))*(c(n,k,l,4)*s_in_min(n,k,l,4))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,5)))*(c(n,k,l,5)*s_in_min(n,k,l,5))&
                  + (0.5D0-sign(0.5D0,c(n,k,l,6)))*(c(n,k,l,6)*s_in_min(n,k,l,6))
             if(abs(c_out_sum(n))<CNST_EPS_ZERO) then
                wrk(n,k,l,s_out_k_min) = s(n,k,l)
                wrk(n,k,l,s_out_k_max) = s(n,k,l)
             else
                wrk(n,k,l,s_out_k_min) = ( &
                     s(n,k,l)-c_qin_sum_max(n)&
                     -s_m1_k_max(n)*(1.0D0-c_in_sum(n)-c_out_sum(n)+d(n,k,l)) &
                     )/c_out_sum(n)
                wrk(n,k,l,s_out_k_max) = ( &
                     s(n,k,l)-c_qin_sum_min(n)&
                     -s_m1_k_min(n)*(1.0D0-c_in_sum(n)-c_out_sum(n)+d(n,k,l)) &
                     )/c_out_sum(n)
             end if
          end do
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do l=1,ADM_LALL_PL
          do k = 1, ADM_kall
             ...
             c_qin_sum_max_pl=0
             do itmp=1,ADM_XTMS_K
                c_qin_sum_max_pl=c_qin_sum_max_pl+&
                     (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1,k,l)))&
                     *(c_pl(ADM_GMIN_PL+itmp-1,k,l)*s_in_max_pl(ADM_GMIN_PL+itmp-1,k,l,1))
             enddo
             c_qin_sum_min_pl=0
             do itmp=1,ADM_XTMS_K
                c_qin_sum_min_pl=c_qin_sum_min_pl+&
                     (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1  ,k,l)))&
                     *(c_pl(ADM_GMIN_PL+itmp-1  ,k,l)*s_in_min_pl(ADM_GMIN_PL+itmp-1  ,k,l,1))
             enddo
    ...
    do l=1,ADM_lall
       ...
       do k = 1, ADM_kall
          ...
          do n=nstart,nend
             sa(n,k,l,ADM_AI) &
                  =(0.5D0-sign(0.5D0,c(n,k,l,1)))&
                  *min(max(sa(n,k,l,ADM_AI),s_in_min(n,k,l,1)),s_in_max(n,k,l,1))&
                  +(0.5D0+sign(0.5D0,c(n,k,l,1)))&
                  *min(max(sa(n,k,l,ADM_AI),s_in_min(n+1,k,l,4)),s_in_max(n+1,k,l,4))
	  end do
	  !
          nstart = suf(ADM_gmin-1,ADM_gmin-1)
          do n=nstart,nend
             sa(n,k,l,ADM_AIJ) &
                  =(0.5D0-sign(0.5D0,c(n,k,l,2)))&
                  *min(max(sa(n,k,l,ADM_AIJ),s_in_min(n,k,l,2)),s_in_max(n,k,l,2))&
                  +(0.5D0+sign(0.5D0,c(n,k,l,2)))&
                  *min(max(sa(n,k,l,ADM_AIJ),s_in_min(n+1+ADM_gall_1d,k,l,5)),s_in_max(n+1+ADM_gall_1d,k,l,5))
          end do
	  !
          nstart = suf(ADM_gmin  ,ADM_gmin-1)
          do n=nstart,nend
             sa(n,k,l,ADM_AJ) &
                  =(0.5D0-sign(0.5D0,c(n,k,l,3)))&
                  *min(max(sa(n,k,l,ADM_AJ),s_in_min(n,k,l,3)),s_in_max(n,k,l,3))&
                  +(0.5D0+sign(0.5D0,c(n,k,l,3)))&
                  *min(max(sa(n,k,l,ADM_AJ),s_in_min(n+ADM_gall_1d,k,l,6)),s_in_max(n+ADM_gall_1d,k,l,6))
          end do
       end do
    end do
    if(ADM_prc_me==ADM_prc_pl) then
    ...
    if(ADM_prc_me==ADM_prc_pl) then
       do l=1,ADM_LALL_PL
          do k = 1, ADM_kall
             ...
             tmp1=0
             do itmp=1,ADM_XTMS_K
                tmp1=tmp1+flx_h_pl(ADM_GMIN_PL+itmp-1,k,l)*sa_pl(ADM_GMIN_PL+itmp-1,k,l)
             enddo
             scl_pl(ADM_GSLF_PL,k,l)=tmp1*fact
...
!!$                  ) * fact
    ...
    return
    !
  end subroutine OPRT_divergence2_rev !XTMS
  !-----------------------------------------------------------------------------
  subroutine OPRT_divergence2_all_rev( & !XTMS
       scl,   scl_pl,       &
       s,     s_pl,         &
       flx_h, flx_h_pl,     &
       c,     c_pl,         &
    ...
    do nq=1, nqmax
       ...
       do l=1,ADM_lall
          ...
          do k = 1, ADM_kall
             ...
             do n=nstart,nend
                !
                if(c(n  ,k,l,2)<=0.0D0) then
                   s_in_min(n,k,l,2,nq) = min(s(n,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),s(n+1,k,l,nq),s(n+ADM_gall_1d,k,l,nq))
                   s_in_max(n,k,l,2,nq) = max(s(n,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),s(n+1,k,l,nq),s(n+ADM_gall_1d,k,l,nq))
                else
                   s_in_min(n+1+ADM_gall_1d,k,l,5,nq) = min(s(n,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),&
                        s(n+1,k,l,nq),s(n+ADM_gall_1d,k,l,nq))
                   s_in_max(n+1+ADM_gall_1d,k,l,5,nq) = max(s(n,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),&
                        s(n+1,k,l,nq),s(n+ADM_gall_1d,k,l,nq))
                end if
                !
             end do
             ...
             do n=nstart,nend
                if(c(n  ,k,l,3)<=0.0D0) then
                   s_in_min(n,k,l,3,nq) = min(s(n,k,l,nq),s(n+ADM_gall_1d,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),s(n-1,k,l,nq))
                   s_in_max(n,k,l,3,nq) = max(s(n,k,l,nq),s(n+ADM_gall_1d,k,l,nq),s(n+1+ADM_gall_1d,k,l,nq),s(n-1,k,l,nq))
                else
                   s_in_min(n+ADM_gall_1d,k,l,6,nq) = min(s(n,k,l,nq),s(n+ADM_gall_1d,k,l,nq),&
                        s(n+1+ADM_gall_1d,k,l,nq),s(n-1,k,l,nq))
                   s_in_max(n+ADM_gall_1d,k,l,6,nq) = max(s(n,k,l,nq),s(n+ADM_gall_1d,k,l,nq),&
       ...
       if(ADM_prc_me==ADM_prc_pl) then
          do l=1,ADM_LALL_PL
             do k = 1, ADM_kall
                ...
                do n=ADM_GMIN_PL,ADM_GMAX_PL
                   if(c_pl(n,k,l)<=0.0D0) then
                      s_in_min_pl(n,k,l,1,nq) = min(s_pl(ADM_GSLF_PL,k,l,nq),s_pl(n,k,l,nq),s_pl(nm1(n),k,l,nq),s_pl(np1(n),k,l,nq))
                      s_in_max_pl(n,k,l,1,nq) = max(s_pl(ADM_GSLF_PL,k,l,nq),s_pl(n,k,l,nq),s_pl(nm1(n),k,l,nq),s_pl(np1(n),k,l,nq))
                   else
                      s_in_min_pl(n,k,l,2,nq) = min(s_pl(ADM_GSLF_PL,k,l,nq),s_pl(n,k,l,nq),s_pl(nm1(n),k,l,nq),s_pl(np1(n),k,l,nq))
                      s_in_max_pl(n,k,l,2,nq) = max(s_pl(ADM_GSLF_PL,k,l,nq),s_pl(n,k,l,nq),s_pl(nm1(n),k,l,nq),s_pl(np1(n),k,l,nq))
                   end if
                end do
             end do
             !
       ...
       do l=1,ADM_lall
          ...
          do k = 1, ADM_kall
             ...
             nend   = suf(ADM_gmax+1,ADM_gmax+1)
             do n = nstart,nend
                s_m1_k_min(n) = min(s_in_min(n,k,l,1,nq),s_in_min(n,k,l,2,nq),s_in_min(n,k,l,3,nq),&
                     s_in_min(n,k,l,4,nq),s_in_min(n,k,l,5,nq),s_in_min(n,k,l,6,nq))
                if(s_m1_k_min(n)==CNST_MAX_REAL) s_m1_k_min(n) = s(n,k,l,nq)
                !        s_m1_k_min(n) = max(SMALL_ZERO,s_m1_k_min(n))
                s_m1_k_max(n) = max(s_in_max(n,k,l,1,nq),s_in_max(n,k,l,2,nq),s_in_max(n,k,l,3,nq),&
                     s_in_max(n,k,l,4,nq),s_in_max(n,k,l,5,nq),s_in_max(n,k,l,6,nq))
                if(s_m1_k_max(n)==-CNST_MAX_REAL) s_m1_k_max(n) = s(n,k,l,nq)
                !
             end do
       ...
       if(ADM_prc_me==ADM_prc_pl) then
          do l=1,ADM_LALL_PL
             do k = 1, ADM_kall
                s_m1_k_min_pl &
                     = min(s_in_min_pl(ADM_GMIN_PL,k,l,1,nq),s_in_min_pl(ADM_GMIN_PL+1,k,l,1,nq),&
                     s_in_min_pl(ADM_GMIN_PL+2,k,l,1,nq),s_in_min_pl(ADM_GMIN_PL+3,k,l,1,nq),s_in_min_pl(ADM_GMIN_PL+4,k,l,1,nq))
                if(s_m1_k_min_pl== CNST_MAX_REAL) s_m1_k_min_pl=s_pl(ADM_GSLF_PL,k,l,nq)
                !          s_m1_k_min_pl = max(SMALL_ZERO,s_m1_k_min_pl)
                s_m1_k_max_pl &
                     = max(s_in_min_pl(ADM_GMIN_PL,k,l,1,nq),s_in_min_pl(ADM_GMIN_PL+1,k,l,1,nq),&
                ...
                do itmp=1,ADM_XTMS_K
                   c_qin_sum_max_pl=c_qin_sum_max_pl+&
                        (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1,k,l)))&
                        *(c_pl(ADM_GMIN_PL+itmp-1,k,l)*s_in_max_pl(ADM_GMIN_PL+itmp-1,k,l,1,nq))
                enddo
                c_qin_sum_min_pl=0
                do itmp=1,ADM_XTMS_K
                   c_qin_sum_min_pl=c_qin_sum_min_pl+&
                        (0.5D0-sign(0.5D0,c_pl(ADM_GMIN_PL+itmp-1  ,k,l)))&
                        *(c_pl(ADM_GMIN_PL+itmp-1  ,k,l)*s_in_min_pl(ADM_GMIN_PL+itmp-1  ,k,l,1,nq))
                enddo
          ...
          do l=1,ADM_LALL_PL
             do k = 1, ADM_kall
                ...
                do itmp=1,ADM_XTMS_K
                   tmp1=tmp1+flx_h_pl(ADM_GMIN_PL+itmp-1,k,l)*sa_pl(ADM_GMIN_PL+itmp-1,k,l)
                enddo
                scl_pl(ADM_GSLF_PL,k,l,nq)=tmp1*fact
...
!!$                  ) * fact
   ...
   return
    !
  end subroutine OPRT_divergence2_all_rev !XTMS
  !
#endif ! end of XTMS modules
