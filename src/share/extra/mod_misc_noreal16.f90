...
contains
  ...
  function MISC_triangle_area_q( &
    ...
    if ( first ) then
       write(*,*) '############################################################'
       write(*,*) '# WARNING                                                  #'
       write(*,*) '#      This subroutine is dummy!, is not support REAL(16). #'
       write(*,*) '#                                                          #'
       write(*,*) '############################################################'
    ...
    if ( .not. present(critical)) then
       ...
       epsi = real(critical,kind=16)
    endif

    if(trim(polygon_type)=='ON_PLANE') then
       !---- Note : On a plane,
       !----        area = | ourter product of two vectors |.
       v1(ix:iz)= real(b(ix:iz),kind=16) - real(a(ix:iz),kind=16)
       v2(ix:iz)= real(c(ix:iz),kind=16) - real(a(ix:iz),kind=16)

       w(ix) = v1(iy)*v2(iz)-v2(iy)*v1(iz)
       w(iy) = v1(iz)*v2(ix)-v2(iz)*v1(ix)
       w(iz) = v1(ix)*v2(iy)-v2(ix)*v1(iy)

       area8 = 0.5E0_8 * sqrt(w(ix)*w(ix)+w(iy)*w(iy)+w(iz)*w(iz))

       a8(ix:iz) = real(a(ix:iz),kind=16)
       len = sqrt( a8(ix)*a8(ix)+a8(iy)*a8(iy)+a8(iz)*a8(iz) )
       area8 = area8 * (radius/len)*(radius/len)

       area = real(area8,kind=16)
    ...
    elseif(trim(polygon_type)=='ON_SPHERE') then
       ...
       !---- NOTE : On a unit sphere,
       !----        area = sum of angles - pi.
       v01(ix:iz)=real(a(ix:iz),kind=16)
       v11(ix:iz)=real(b(ix:iz),kind=16)-real(a(ix:iz),kind=16)
       v21(ix:iz)=real(c(ix:iz),kind=16)-real(a(ix:iz),kind=16)
       !----
       v02(ix:iz)=real(b(ix:iz),kind=16)
       v12(ix:iz)=real(a(ix:iz),kind=16)-real(b(ix:iz),kind=16)
       v22(ix:iz)=real(c(ix:iz),kind=16)-real(b(ix:iz),kind=16)
       !----
       v03(ix:iz)=real(c(ix:iz),kind=16)
       v13(ix:iz)=real(a(ix:iz),kind=16)-real(c(ix:iz),kind=16)
       v23(ix:iz)=real(b(ix:iz),kind=16)-real(c(ix:iz),kind=16)
       !----
       r_v01_x_v01&
            =1.0_RP/(v01(ix)*v01(ix)+v01(iy)*v01(iy)+v01(iz)*v01(iz))
       fac11=(v01(ix)*v11(ix)+v01(iy)*v11(iy)+v01(iz)*v11(iz))&
       ...
       ang(3)=acos(ang(3))
       !----
       area8=(ang(1)+ang(2)+ang(3)-pi)*radius*radius
